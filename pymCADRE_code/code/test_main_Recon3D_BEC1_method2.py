...
# model
model = io.read_sbml_model('../pre_processing/dataset/Recon3D_with_VBOF.xml')
...
U = list(U['Scores'])
# confidence scores
confidence_scores = pd.read_csv('../pre_processing/dataset/Recon3D_confidence_scores.csv')
...
GM, C, NC, P, Z, model_C = rank_reactions(model, G, U, confidence_scores, [], method=2)
...
if genericStatus:
    ...
    t0 = process_time()
    PM, cRes = prune_model(GM, P, C, Z, eta, precursorMets, salvage_check=1, method=2)
...
#########################################
io.write_sbml_model(PM, "python_recon3D_BEC1_method2.xml")
