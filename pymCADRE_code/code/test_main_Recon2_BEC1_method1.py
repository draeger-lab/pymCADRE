...
# model
model = io.read_sbml_model('../pre_processing/dataset/recon_2.2_Kiel.xml')
...
##############################################
# Recon2 pre-processing
##############################################
# rename genes
for idx, g in enumerate(model.genes):
    if 'HGNC:HGNC' in g.id:
        model.genes[idx].id = g.id.replace('HGNC:HGNC', 'HGNC')
    else:
        model.genes[idx].id = g.id.replace(':','_')
# rename GPRS
for idx2, r in enumerate(model.reactions):
    model.reactions[idx2].gene_reaction_rule = r.gene_reaction_rule.replace(':', '_')

# exclude duplicated genes
model.genes = model.genes[:1691]
##############################################
...
#########################################
io.write_sbml_model(PM, "python_recon2_BEC1_method1.xml")
